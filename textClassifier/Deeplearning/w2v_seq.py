...
def tsne():
    ...
    for i in y:
        ...
        xtrain.append(temp)
    import matplotlib.pyplot as plt
    '''X_tsne = TSNE(n_components=2, learning_rate=100, init='pca', random_state=501).fit_transform(xtrain)
    x_min, x_max = X_tsne.min(0), X_tsne.max(0)
    X_norm = (X_tsne - x_min) / (x_max - x_min)  # 归一化
    plt.figure(figsize=(8, 8))
    for i in range(X_norm.shape[0]):
        plt.text(X_norm[i, 0], X_norm[i, 1], str(ytrain[i]), color=plt.cm.Set1(ytrain[i]),
    ...
    plt.scatter(tsne[:, 0], tsne[:, 1], c=ytrain)
    plt.colorbar()
    plt.show()'''
    # coding='utf-8'
    """# 一个对S曲线数据集上进行各种降维的说明。"""
    ...
    tsne = manifold.TSNE(n_components=n_components, init='pca', random_state=0,n_iter=10000,perplexity=300,learning_rate=5000)
    Y1 = tsne.fit_transform(xtrain)  # 转换后的输出


    ax = fig.add_subplot(2, 1, 2)
    plt.scatter(Y1[:, 0], Y1[:, 1], c=ytrain, cmap=plt.cm.Spectral)
